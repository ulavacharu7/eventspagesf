...
function applyEventStartTime(d: Date, timeStr?: string | null) {
  if (!timeStr || !timeStr.trim()) {
    d.setHours(0, 0, 0, 0);
    return;
  }
...
export function getEventStartTimestamp(event: {
  ...
  if (!event || !event.startDate || !event.startDate.trim()) return 0;
  const raw = event.startDate.trim();
  ...
  try {
    ...
    if (/^\d{4}[-/]\d{1,2}[-/]\d{1,2}/.test(raw)) {
      ...
      if (!isNaN(d.getTime())) {
        applyEventStartTime(d, event.startTime);
        return d.getTime();
      }
    }
    ...
    // 2. Check for explicit 4-digit year (e.g. "Sun, 6 Sep 2026")
    const yearMatch = raw.match(/\b(20\d{2})\b/);
    const year = yearMatch ? parseInt(yearMatch[1], 10) : new Date().getFullYear();

    // 3. Match "6 Sep", "Sun, 6 Sep", "Sep 6", "September 6", "2 Sep"
    ...
    if (dayMonthMatch) {
      ...
      if (monthIdx !== undefined && !isNaN(day)) {
        ...
        applyEventStartTime(eventDate, event.startTime);
    ...
    if (!isNaN(fallbackDate.getTime())) {
      ...
      applyEventStartTime(fallbackDate, event.startTime);
      return fallbackDate.getTime();
    }
  } catch (err) {
    console.error('getEventStartTimestamp parse error:', err);
  }
