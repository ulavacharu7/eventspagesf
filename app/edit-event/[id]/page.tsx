...
export default function EditEventPage({ params }: PageProps) {
  ...
  // Protect route: user must be authenticated to edit/host an event
  useEffect(() => {
    try {
    ...
    async function loadEvent() {
      ...
      try {
        ...
        if (data && data.event) {
          ...
          setTicketPrice(ev.price || 'Free');
          setRequireApproval(!!ev.requireApproval);
          setCapacity(ev.capacity || 'Unlimited');
          setCalendarType(ev.calendarType || 'Personal Calendar');
          setVisibility(ev.visibility || 'Public');
          setUploadedImageSrc(ev.coverImage || null);
          setCurrentThemeIdx(ev.themeIdx !== undefined ? ev.themeIdx : 0);
          setCurrentFont(ev.font || 'Default');
          
          if (ev.customFields) {
            try {
              setCustomFields(JSON.parse(ev.customFields));
            } catch (e) {
              console.error(e);
            }
          }
          if (ev.speakers) {
            try {
              setSpeakers(JSON.parse(ev.speakers));
            } catch (e) {
              console.error(e);
            }
          }
      ...
      <div className="w-full max-w-5xl mx-auto pt-12 sm:pt-16 md:pt-20 pb-12 px-4 sm:px-8 flex-1 flex flex-col justify-center relative z-10">
        ...
        <div className="flex flex-col gap-2.5 pb-4 mb-6 border-b border-white/10">
          <nav className="flex items-center gap-1.5 text-[11px] font-mono text-white/40">
            ...
            <span className="opacity-30">/</span>
            <span className="text-white/80">Edit Event</span>
          </nav>
        ...
        {isSubmitted ? (
          <div className="bg-[#1c1c1f] border border-[#2e2e34] rounded-2xl p-8 text-center max-w-md mx-auto flex flex-col items-center gap-4 shadow-2xl">
            ...
            <h2 className="text-2xl font-bold text-white">Event Updated!</h2>
            <p className="text-xs text-[#a1a1aa] leading-relaxed">
              Your event <strong className="text-white">&quot;{eventName || 'Untitled Event'}&quot;</strong> has been successfully updated. Redirecting to dashboard...
            </p>
          </div>
        ) : (
